...
export function renderHeatmapRingSvg(
  ...
  const monthLabels = weeks
    .map((_, i) => {
      ...
      return `<text x="${x}" y="${y}" text-anchor="middle" dominant-baseline="central" fill="${theme.subtext}" font-size="9" font-family="Inter,system-ui,sans-serif">${MONTH_NAMES[monthKey]}</text>`;
    })
    .join("");
