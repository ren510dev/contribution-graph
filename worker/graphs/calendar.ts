...
export function renderCalendarSvg(
  ...
  weeks.forEach((week, wi) => {
    for (const day of week.contributionDays) {
      ...
      if (!seen.has(key)) {
        seen.add(key);
        months.push({ name: monthNames[d.getMonth()], weekIdx: wi });
      }
    }
  });
  ...
  const monthLabels = months
    .map(
      (m) =>
        `<text x="${ox + labelWidth + m.weekIdx * step}" y="${gridTopY - 6}" fill="${theme.subtext}" font-size="10" font-family="Inter,system-ui,sans-serif">${m.name}</text>`,
    )
    .join("");
