...
export function renderStatsBarSvg(
  ...
  const dowBars = dowCounts
    .map((count, i) => {
      ...
      const barCx = x + bw / 2;
      const countLabelY = y - 6;
      const countLabel = isBest
        ? `<text x="${barCx}" y="${countLabelY}" fill="${theme.line}" font-size="9" font-weight="600" font-family="Inter,system-ui,sans-serif" text-anchor="middle">${count}</text>`
        : "";
      return `<rect x="${x}" y="${y}" width="${bw}" height="${bh}" rx="4" fill="${isBest ? theme.line : theme.point}" opacity="${isBest ? 1 : 0.45}"/>
      <text x="${barCx}" y="${barY + barH + 16}" fill="${theme.subtext}" font-size="10" font-family="Inter,system-ui,sans-serif" text-anchor="middle">${dowLabels[i]}</text>
      ${countLabel}`;
    })
    .join("");
