      ...
      <div className="bg-white p-5 sm:p-6">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-8">
            {GRAPH_TYPES.map((g) => (
              ...
              >
                ...
                <span
                  className={`text-[11px] font-medium leading-tight ${graphType === g.id ? "text-bordeaux-950" : "text-bordeaux-700/70"}`}
                >
                  {g.name}
                </span>
                {graphType === g.id && (
        ...
        <div className="mb-6 overflow-hidden rounded-xl border border-bordeaux-200/50 shadow-sm">
          <div className="flex items-center gap-2 border-b border-bordeaux-200/50 bg-bordeaux-50/50 px-4 py-2.5">
            ...
            <div className="flex-1 rounded-md bg-white px-3 py-1 text-[10px] font-mono text-bordeaux-700/50 border border-bordeaux-200/50 truncate">
              {svgUrl.replace(baseUrl, "your-domain.com")}
            </div>
            <a
              href={svgUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="shrink-0 rounded-md bg-bordeaux-50 px-2.5 py-1 text-[10px] font-medium text-bordeaux-700/70 transition-colors hover:bg-bordeaux-100 hover:text-bordeaux-600"
            >
              Open SVG
            </a>
          </div>
        ...
        <div className="space-y-2">
          {[
            ...
            >
              <span className="w-16 shrink-0 text-[10px] font-semibold tracking-wider text-bordeaux-700/60 uppercase">
                {row.label}
              </span>
              <code className="min-w-0 flex-1 truncate font-mono text-[11px] text-bordeaux-700">
                {row.value}
              </code>
              <button
                onClick={() => handleCopy(row.value, row.key)}
                className={`shrink-0 rounded-lg px-3.5 py-1.5 text-[11px] font-semibold transition-all duration-200 ${
                  copied === row.key
